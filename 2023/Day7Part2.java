...
public class Day7Part2
{
    ...
    public static int analyzeHand(String hand)
    {
        ...
        return tryJokers(numJokers,hand);
    }

    public static int tryJokers(int numJokers, String hand)
    {
        String[] values = {"A","K","Q","T","9","8","7","6","5","4","3","2"};
        if(numJokers==0)
        {
            return analyzeHandNoJokers(hand);
        }

        int max = -1;
        for(int i=0;i<values.length;i++)
        {
            String handCopy = hand.replaceFirst("J", values[i]);
            max = Math.max(max,tryJokers(numJokers-1,handCopy));
        }
        return max;
    }

    public static int analyzeHandNoJokers(String hand)
    {
        ...
        System.out.println("Something went wrong no jokers");
        return -1;
    }
}